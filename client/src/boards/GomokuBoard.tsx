...
import type { GomokuState } from '@qi/shared';
...
export function GomokuBoard({ state, interactive, onMove }: Props) {
  ...
  useEffect(() => {
    ...
    // 星位
    const stars = n === 15 ? [[3, 3], [11, 3], [7, 7], [3, 11], [11, 11]] : [];
    ...
    if (state.lastMove) {
      ...
      ctx.beginPath();
      ctx.arc(MARGIN + x * cell, MARGIN + y * cell, cell * 0.14, 0, Math.PI * 2);
      ctx.fillStyle = state.board[y][x] === 1 ? '#e74c3c' : '#e74c3c';
      ctx.fill();
    }
