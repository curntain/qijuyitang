...
| 中国象棋 | ✅ | ✅ | 完整走法与将军判定 |
| 国际象棋 | ✅ | ✅ | 完整规则（含王车易位、升变） |
| 四国军棋 | ✅ | — | 2~4 人，战前可自动/手动布阵（行营开局留空），迷雾（敌方棋子显示 ?），中央铁路与工兵飞行，夺旗淘汰，4 人时南北组队对抗东西 |
...
- 🔄 对局内支持悔棋（需对方同意；四国军棋 2~4 人也支持）、认输、断线重连
- 📊 每人每棋种的胜负平战绩统计
- 📱 移动端自适应 + 落子音效

## 📝 更新日志

### v1.1.0 · 四国军棋全面重做（2026-08-29）

- 🎨 四国军棋棋盘重制：木质纯色底、6×5 阵营、行营开局留空、中央"井"字铁路与 9 个十字格、首排兵站铁路接轨、四角规则说明
- ↩️ 四国军棋支持悔棋：上一手本人申请、当前行棋方确认（2~4 人通用）
- 🔍 棋子与界面优化：尺寸与间距调整，对局更清晰
...
不想装 Node、不想敲命令的朋友,直接到本仓库的 **Releases →「安装包下载」** 页面下载对应平台的安装包:
...
**安卓说明**:APK 是纯客户端,需要有一台电脑/服务器先运行本平台,且构建 APK 时要写入服务器地址。仓库已内置自动构建流水线:到 **Actions → Build installers → Run workflow**,在 `server_url` 里填你的服务器地址(如 `http://192.168.1.100:3001`),构建完成后 APK 自动出现在「安装包下载」里。其实同一局域网内,手机直接用浏览器打开 `http://局域网IP:3001` 体验完全相同,建议优先用浏览器。
